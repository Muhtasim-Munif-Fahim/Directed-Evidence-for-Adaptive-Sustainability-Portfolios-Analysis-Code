...
- Local files:
  - `data/raw/wb_income_classification.csv`
  - `data/raw/wb_region_classification.csv`
- Source: World Bank Country API metadata:
  `https://api.worldbank.org/v2/country?format=json&per_page=400`
- API fields used: country `id` (ISO3), `name`, `region.id`, and
  `incomeLevel.id`.
- The local CSVs normalize World Bank API codes into the analysis labels used by
  the scripts:
  - `income_level`: `LIC`, `LMIC`, `UMIC`, `HIC`
  - `region`: `EAP`, `ECA`, `LAC`, `MEA`, `NAC`, `SAS`, `SSA`
